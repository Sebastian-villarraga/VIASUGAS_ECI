...
async function initManifiestos() {
  ...
  // ?? NUEVO: aplicar filtro automàtico
...
async function cargarCatalogosManifiesto() {
  ...
  if (!data) {
    showToast("No se pudieron cargar los catàlogos de manifiestos", "error");
    return;
  }
...
async function activarEdicionDetalle() {
  ...
  if (
    ...
    showToast(
      `Este manifiesto està siendo editado por ${manifiestosEditando[id]}`,
      "warning"
    );
...
function actualizarTrackingEstado(estado) {
  ...
  // ?? obtener posiciòn REAL del step
...
function mostrarBannerEdicion(
  ...
  banner.innerHTML =
    `?? ${usuario} està editando este manifiesto`;
